...
#    stock_ids = [symbol('AAPL'), symbol('AMZN'), symbol('ATVI'), symbol('AAMRQ'), symbol('CSCO'), symbol('DISH'), symbol('INTC'), symbol('LBTY_B'), symbol('MAT'), symbol('MSFT'), symbol('PYPL'), symbol('TXN'), symbol('VIAB'), symbol('SYMC')]
    ...
    stock_ids = [symbol('ACI'), symbol('CCK')]#, symbol('BDK'), symbol('XTO'), symbol('GR'), symbol('TRA'), symbol('GENZ'), symbol('MI'), symbol('EP'), symbol('SII'), symbol('NVLS'), symbol('ACI'), symbol('PDE'), symbol('CBE'), symbol('ALTR'), symbol('DD'), symbol('DE'), symbol('FWLT'), symbol('CCK')]
    

    #Simulating that we are trading on robinhood
    set_commission(commission.PerShare(cost=0.00, min_trade_cost=0.00)) # cost changed from 0 to 10 & trade changed from 0 to 100 by Alfred
    ...
    schedule_function(weekly_assessment, date_rules.week_start(3), time_rules.market_open(hours=0,minutes=5))
...
def update_garch(equity, context, data):
    ...
    #Set input variables (1000 changed to 10 by Alfred)
